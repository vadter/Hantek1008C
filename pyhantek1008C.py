...
import copy
import sys, time, struct, numpy as np
...
class Hantek1008CRaw:
    ...
    dictSR = {100_000: 300_000, 200_000: 250_000, 500_000: 100_000,
              1_000_000: 50_000, 2_000_000: 25_000, 5_000_000: 10_000,
              10_000_000: 5_000}
    ...
    def __send_cmd(self, cmd_id: int, parameter: Union[bytes, List[int], str] = b'',
                   ...
                   sec_till_response_request: float = 0., sec_till_start: float = 0.) -> bytes:
    ...
    def __send_c6_a6_command(self, parameter: int) -> bytes:
        ...
        for _ in range(sample_packages_count):
            ...
            response = self.__send_cmd(0xa6, parameter = [parameter],
                                       response_length = 64,
                                       echo_expected = False,
                                       sec_till_response_request = 0.00,
                                       sec_till_start = 0.00)
        ...
        # print('__send_c6_a6_command: %1.3f' % (dt0))
    ...
    def __send_set_time_div(self, ns_per_div: int = 500000) -> None:
        ...
        self.samplerate = self.dictSR[ns_per_div]
        
        self.time = np.linspace(0., 499, 500) / self.samplerate
    ...
    def __send_set_active_channels(self, active_channels: List[int]) -> None:
        """
        Activates only the channels that are in the list
    ...
    def request_samples_burst_mode(self, mode: str = "volt"
        ...
        self.__send_cmd(0xe6, parameter = [0x01],
                        echo_expected = False,
                        response_length = 10)
        
        self.__send_cmd(0xa4, parameter = [0x01], sec_till_response_request = 0.001)
        ...
        # these two commands are not necessarily required
        self.__send_cmd(0xe4, parameter = [0x01])
        self.__send_cmd(0xe6, parameter = [0x01], echo_expected = False,
                        response_length = 10)
        
        sample_shorts = Hantek1008CRaw.__from_bytes_to_shorts(sample_response)
       
        data = (np.array(sample_shorts, dtype = float) - 2048. + 128.) / 4096. * 10. * 4.
        data = data.reshape((data.size // 8, 8)).T
        
        Ch1 = data[0] * self.__vertical_scale_factors[0]
        Ch2 = data[1] * self.__vertical_scale_factors[1]
        Ch3 = data[2] * self.__vertical_scale_factors[2]
        Ch4 = data[3] * self.__vertical_scale_factors[3]
        Ch5 = data[4] * self.__vertical_scale_factors[4]
        Ch6 = data[5] * self.__vertical_scale_factors[5]
        Ch7 = data[6] * self.__vertical_scale_factors[6]
        Ch8 = data[7] * self.__vertical_scale_factors[7]
...
#    def request_samples_burst_mode(self) -> Dict[int, List[int]]:
    def request_shorts_burst_mode(self, mode: str = "volt"
                                   ) -> List[int]:
        """get the data"""
...
#             self.__send_cmd(0xa3, parameter=[18])
    ...
    def __from_bytes_to_shorts(data: bytes) -> List[int]:
        ...
        data_unpack = struct.unpack('H' * (len(data) // 2), data)
...
#                                   ) -> Dict[int, List[int]]:
        assert self.__zero_offset_shift_compensation_channel is None, \
            "zero offset shift compensation is not implemented for burst mode"
        raw_per_channel_data = Hantek1008CRaw.request_shorts_burst_mode(self)
        return raw_per_channel_data
